
package pascaltoc.lexer;


public enum TokenType {
    Number, Keyword, Operator, String, Assignment, Equal, 
    NotEqual, Program, Begin, End, Or, Not,And, Mod, Div, 
    Case, Integer, Real, Input, Output,Var, While, Plus, 
    Minus, EndPoint, Uses, Function, TwoPoints, Boolean, 
    StringType, Character, Comma, OpenParentheses, CloseParentheses, 
    NewLine, Apostrophe, Do, If, Then, Else, Lesser, Greater, Exclamation
    
}
